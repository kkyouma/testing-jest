...
describe('analyzeArray', () => {
  ...
  test('negative numbers', () => {
    ...
    expect(result).toEqual({ average: -4, min: -8, max: -1, length: 5 });
  });
  test('decimals', () => {
    const result = analyzeArray([2.5, 1.8, 3.2, 4.7, 2.1]);
    expect(result).toEqual({ average: 2.86, min: 1.8, max: 4.7, length: 5 });
  });
  test('empty', () => {
  ...
  test('just one element', () => {
    ...
    expect(result).toEqual({ average: 10, min: 10, max: 10, length: 1 });
  });
});
